...
public class WalkingSoundNotification : MonoBehaviour {
	...
	void OnGUI(){
		...
		if(otherPlayer.GetComponent<Pilot>().currentGolem != null){
			otherGolemVelocity = otherPlayer.GetComponent<Pilot>().currentGolem.GetComponent<Rigidbody2D>().velocity;
		}else{
			otherGolemVelocity = Vector2.zero;
		}
		if(Vector2.Distance(transform.position,otherPlayer.position) > GetComponent<Camera>().orthographicSize+.75f && 
												 otherGolemVelocity != Vector2.zero){
